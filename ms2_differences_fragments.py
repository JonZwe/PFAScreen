...
from utils import ismembertol                 # Array comparison within an absolute tolerance
...
def ms2_differences_fragments(
    ...
    diffs = df_fragment_differences['fragment_difference'].to_list()
    m_diffs = frags_to_mass(diffs)

    # NOTE: added as preliminary solution to convert lists to np.arrays
    spec_mz_list = [np.array(spec) for spec in spec_mz_list]
    spec_intens_list = [np.array(spec) for spec in spec_intens_list]
    ...
    def get_diagnostic_fragments(df_diagnostic_fragments, spec_mz_list_corr_fil, polarity):
        
        dia_frags = df_diagnostic_fragments.copy()

        dia_frags = dia_frags[dia_frags['polarity'] == polarity]

        dia_frags['F_containing'] = dia_frags['fragment_formula'].str.contains('F') # CAUTION: not robust, might contain other elements with letter F
        ...
        for n in range(len(spec_mz_list_corr_fil)):
            ...
            idx_dia_frags[n], idx_dia_formula[n], _, _ = ismembertol(spec_mz_list_corr_fil[n], dia_frags['fragment_mass'], mass_tolerance)
    ...
    idx_dia_frags, spec_mz_dias, spec_intens_dias, spec_formula_dias, n_dia_frags_f_contain, n_dia_frags = get_diagnostic_fragments(df_diagnostic_fragments, 
                                                                                                                                    spec_mz_list_corr_fil, 
                                                                                                                                    polarity)
    ...
    """
    NOTE: this must be replaces by the networkx approach as it has bugs, and it not robust
    ...
    idx_MS1_hit = index[idx_diff_or_dia_prec]
    ...
    # NOTE: HERE IS BUG: Sometimes the header CF2 is present more than once! This needs to be fixes, as the html reporting cannot handle that
    append_str_frag = 'mz_'
    ...
    Df_fin = Df_fin.set_index(idx_MS1_hit.astype('int'))
